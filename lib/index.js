...
const { lstatSync, readFileSync: readFile } = require('node:fs')
...
// true when a bundled dep lives outside the package being packed (hoisted workspace
// installs, sibling node_modules, etc.). those files must be remapped into
// node_modules/<name> so the tarball does not contain ../ escapes.
const isOutsidePackRoot = (rel) => {
  const normalized = normalizePath(rel)
  return normalized === '..' ||
    normalized.startsWith('../') ||
    // istanbul ignore next -- windows cross-drive relative() returns an absolute path
    isAbsolute(normalized)
}

// Walk up from each start directory looking for node_modules/<name>, matching Node's
// resolution of a hoisted install. Used when Arborist loaded the workspace package as
// its own project root and therefore left edgesOut.to === null for root-hoisted deps.
const findInstalledPackage = (startDirs, name) => {
  const rel = join('node_modules', name)
  const seen = new Set()
  for (const start of startDirs) {
    ...
    while (!seen.has(dir)) {
      ...
      try {
        const pkg = JSON.parse(readFile(join(candidate, 'package.json'), { encoding: 'utf8' }))
        let isLink = false
        try {
          isLink = lstatSync(candidate).isSymbolicLink()
        } catch {
          // istanbul ignore next -- package.json was readable so lstat almost always works
        }
        return { path: normalizePath(candidate), isLink, pkg }
      } catch {
        // missing, unreadable, or invalid package.json: keep walking
      }
...
// Minimal tree that gatherBundles can recurse into for a path-resolved hoisted dep.
// isProjectRoot is false so we include the dep's own production/optional dependencies.
const makeSyntheticBundleNode = (found) => {
  const pkg = found.pkg
  ...
  for (const depName of [
    ...
    edgesOut.set(depName, { peer: false, dev: false, to: null })
  }
  const tree = {
    path: found.path,
    package: pkg,
...
class PackWalker extends IgnoreWalker {
  constructor (tree, opts) {
    ...
    this.tree = tree
    this.packRoot = options.packRoot || this.root
  ...
  async gatherBundles () {
    // istanbul ignore if -- duplicate trees are skipped before the bundled walker is created
    if (this.seen.has(this.tree) || (this.tree.path && this.seen.has(this.tree.path))) {
      return
    }

    // add this node to our seen tracker. path is also recorded so synthetic nodes
    // created for hoisted deps cannot recurse forever on cycles.
    this.seen.add(this.tree)
    // istanbul ignore else -- arborist and synthetic nodes always have a path
    if (this.tree.path) {
      this.seen.add(this.tree.path)
    }
    ...
    for (const dep of toBundle) {
      const edge = this.tree.edgesOut && this.tree.edgesOut.get(dep)
      ...
      let node = edge && edge.to
      // Arborist loaded from a workspace package path does not resolve deps that
      // were hoisted to the workspace root. Walk up to find node_modules/<dep>.
      // At the project root, names that are not actual dependencies (no edge)
      // are still skipped, matching prior behavior.
      if (!node) {
        if (this.tree.isProjectRoot && !edge) {
          continue
        }
        const found = findInstalledPackage(
          [this.path, this.tree.path].filter(Boolean),
          dep
        )
        if (!found) {
          continue
        }
        node = makeSyntheticBundleNode(found)
      }
      ...
      // skip packages we have already started bundling. this must happen before
      // walking files: remapped hoisted paths do not cancel the way ../ siblings do,
      // so a second walk would otherwise nest a copy of the same package.
      if (this.seen.has(tree) || (tree.path && this.seen.has(tree.path))) {
        continue
      }
      ...
      // our own result set to deduplicate. only remap when the dep lives outside the *original*
      // package root (hoisted workspace installs). sibling deps under that root still use
      // ../ so existing flattened node_modules layouts stay intact.
      let relativeFrom = normalizePath(relative(this.root, walker.path))
      if (isOutsidePackRoot(normalizePath(relative(this.packRoot, walker.path)))) {
        relativeFrom = normalizePath(join('node_modules', dep))
      }
