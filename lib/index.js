...
const findInstalledPackage = (startDirs, name) => {
  ...
  for (const start of startDirs) {
    if (!start) {
      // istanbul ignore next -- callers pass walker and tree paths
      continue
    }
  ...
  async gatherBundles () {
    ...
    for (const dep of toBundle) {
      ...
      if (!node) {
        ...
        const found = findInstalledPackage([this.path, this.tree.path], dep)
        if (!found) {
          continue
        }
      ...
      // our own result set to deduplicate. hoisted installs live outside this.root, so remap
      // them to node_modules/<name> instead of emitting ../node_modules/... tarball paths.
      let relativeFrom = normalizePath(relative(this.root, walker.path))
      if (isOutsidePackRoot(relativeFrom)) {
